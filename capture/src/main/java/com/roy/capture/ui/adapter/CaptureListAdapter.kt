...
class CaptureListAdapter(list: ArrayList<CaptureData>) :
    BaseQuickAdapter<CaptureData, BaseViewHolder>(
        R.layout.capture_item_capture_list_adapter,
        list
    ) {


    private var itemClick: ((item: CaptureData) -> Unit)? = null

    @SuppressLint("SetTextI18n")
    override fun convert(holder: BaseViewHolder, item: CaptureData) {

        val dataBinding =
            DataBindingUtil.bind<CaptureItemCaptureListAdapterBinding>(holder.itemView)
    ...
    fun itemClickListener(itemClick: ((item: CaptureData) -> Unit)?) {
